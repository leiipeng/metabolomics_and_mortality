...
columns_to_impute <- c(4:5, 7:10, 11:259)
...
# Perform log transformation (+1) on columns 11 to 259
ukb_all_new_imputed_logall <- ukb_all_new_imputed %>%
  mutate(across(11:259, ~ log(. + 1)))
...
# Perform z-score normalization on columns 11 to 259 in the dataframe ukb_all_new_imputed_logall
ukb_all_new_imputed_logall_z <- z_tranb(ukb_all_new_imputed_logall, 11, 259)
...
# Cox regression in total study population
...
covariates <- colnames(ukb_all_new_imputed_logall_z)[c(2:10, 11:14)]
time_var <- colnames(ukb_all_new_imputed_logall_z)[267]
outcome_var <- colnames(ukb_all_new_imputed_logall_z)[268]
metabolite_columns <- colnames(ukb_all_new_imputed_logall_z)[15:259]
...
write.xlsx(results_df, "cox_ukb_all_245results.xlsx", rowNames = FALSE)

# Create volcano plot for 245 metabolic biomarkers in the UK Biobank
# Load necessary packages
library(ggplot2)
library(ggrepel)
library(dplyr)
...
ggsave("cox_ukb_all_volcano_245biomarkers.pdf", plot = volcano_plot1, width = 8, height = 7)

# Extract significant 228 metabolic biomarkers based on FDR-adjusted p-value
significant_metabolites <- results_df %>%
  filter(p.adjust < 0.05) %>%
  pull(metabolite)

# Validate these 228 metabolic biomarkers in the ESTHER Study
...
write.xlsx(new_results_df, "cox_es_all_228results.xlsx", rowNames = FALSE)

# Create volcano plot for 228 metabolic biomarkers in the ESTHER Study
# Load necessary packages
library(ggplot2)
library(ggrepel)
library(dplyr)
...
ggsave("cox_es_all_volcano_228biomarkers.pdf", plot = volcano_plot2, width = 8, height = 7)
