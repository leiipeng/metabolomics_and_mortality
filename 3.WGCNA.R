...
# 1-Extract 95 significant metabolic biomarkers in the UK Biobank
library(readxl)
results228 <- read_excel("cox_es_all_228results.xlsx", sheet = 1, na = "NA")

# Load the necessary library
library(dplyr)

# Filter 95 significant metabolic biomarkers with p.adjust < 0.05
significant_metabolites <- results228 %>%
  filter(p.adjust < 0.05) %>%
  select(metabolite)

# Check if there are 95 biomarkers
if(nrow(significant_metabolites) == 95) {
  # Extract the corresponding biomarker variables
  ukb_biomarkers95 <- ukb_all_new_imputed_logall_z %>%
    select(all_of(significant_metabolites$metabolite))
  
  # View the results
  head(ukb_biomarkers95)
} else {
  print(paste("The number of biomarkers after filtering is not 95, actual number is: ", nrow(significant_metabolites)))
}
...
correlation_matrix <- cor(ukb_biomarkers95, method = "pearson", use = "pairwise.complete.obs")
...
module_biomarkers <- data.frame(
  Biomarker = colnames(ukb_biomarkers95),  # The names of the biomarkers
...
selectTOM <- TOMsimilarityFromExpr(ukb_biomarkers95, power = 6)
dissTOM <- 1 - selectTOM
